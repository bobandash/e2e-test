...
This project uses Fixtures and the Page Object Model (POM) to promote reusable and DRY test code.
...
- **Hide Posts**:
  Unauthenticated users cannot hide posts.
  Authenticated users can hide posts.
- **Upvote Posts**:
- Verified that signed-in users can upvote posts.
- **Comment Post**: Verified that only authenticated users can comment on posts.

### Comments and Jobs Page:

- **Sorted By Newest**: Verified that posts are displayed in descending order by time.

### Auth Page:

- **Username Already Taken**: Verified that users cannot create an account with an existing username.
- **Fail to Login**: Verified that incorrect credentials prevent login.
- **Login Successful**: Verified that successful login redirects users to the posts page.
